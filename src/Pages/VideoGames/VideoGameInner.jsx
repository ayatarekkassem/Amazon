...
const VideoGameInner = () =>{
  const { addToCart } = useContext(BookStoreContext);

  const { id } = useParams();
  const [qty, setQty] = useState(1);

  const VideoGamesData = VideoGamesDatas.find(p => p.id === parseInt(id));
  ...
  const settings = {
    ...
    return(
      ...
      <Slider {...settings}>
        <div>
          <img src={VideoGamesData.image} alt="" />
        </div>
        <div>
        <img src={VideoGamesData.imageTwo} alt="" />

        </div>
        <div>
        <img src={VideoGamesData.imageThree} alt="" />

        </div>
        <div>
        <img src={VideoGamesData.imageFour} alt="" />

        </div>
      </Slider>
        </div>
        {/* <div className="innerPage-img">
          <img src={VideoGamesData.image} alt="" />
        </div> */}
        </div>
        {/* slider */}
        <div className="InnerPage-info">
        <h4>{VideoGamesData.title}</h4>
            <p>{VideoGamesData.para}</p>
            <Rating className="rating" rating={VideoGamesData.rating} reviews={VideoGamesData.reviews} />
               <div className="price">
                  <span>EGP{VideoGamesData.price}</span>
            ...
            />
                  <p className="get"> {VideoGamesData.otherpara}</p>
                  <p>{VideoGamesData.otherparapara}</p>
                  <ul className="cash">
                ...
                <button className="inner-btn"  onClick={() => addToCart(VideoGamesData,qty)}>Add To Cart</button>
